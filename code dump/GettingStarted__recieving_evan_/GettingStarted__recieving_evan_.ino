  ...
  if ( role == role_pong_back )
  {
    ...
    if ( radio.available() )
    {
      ...
      { 
        ...
        printf("Recieved information: x-coord = %d, y-coord = %d,value = %d \n", updates[0],updates[1], updates[2]);
        // Spew it
        printf("Got payload. Printing out the new array (after updates applied):");
        for (int x= 0; x<5; x++){
          printf("\n");
          for (int y = 0; y<5; y++){
            printf(" %d",maze[x][y]);
      ...
      radio.write(&updates, sizeof(unsigned char)*3);
      printf("\n Sent response.\n\r");
